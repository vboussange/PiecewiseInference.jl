...
function inference(infprob;
    ...
    !save_pred && (pred = nothing)
    !save_losses && (losses = nothing)
...
Return a vector of indices to create segments (groups of datapoints), based on
whether `group_size` or `group_nb`.
"""
function get_ranges(;group_size = nothing, group_nb = nothing, datasize)
    ...
    elseif !isnothing(group_nb)
        ...
        throw(ArgumentError("Need to provide whether `group_nb` or `group_size` as keyword argument"))
    end
